...
namespace httpsli::requests::http {
    ...
    HttpRequest HttpRequestConstructor::Construct(const std::string& request_string) {
        ...
        httpsli::http::RequestType request_type = httpsli::http::EnumGetter::GetRequestType(request_type_string);
        ...
        for (; line_index < request_lines.size(); ++line_index){
            ...
            boost::split(header_parts, request_lines[line_index], boost::is_any_of(": "));
